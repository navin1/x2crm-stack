...
LinkedInWidget = (function () {

    function LinkedInWidget(argsDict) {
        var defaultArgs = {
            DEBUG: x2.DEBUG && false,
            lastLinkedInPostId: null,
            screenName: null
        };
    ...
    LinkedInWidget.prototype = auxlib.create(SortableWidget.prototype);

    LinkedInWidget.prototype._loadMoreLinkedInPosts = function (reload) {
        reload = typeof reload === 'undefined' ? false : reload;
        var that = this;
        var data = {
            linkedInFeedAjax: true,
        };
        if (!reload) {
            data.maxLinkedInPostId = this.lastLinkedInPostId;
        }
        if (this.screenName) {
            data.linkedInScreenName = this.screenName;
        }
        $.ajax({
    ...
    LinkedInWidget.prototype._setUpPaginationButtonBehavior = function () {
        var that = this;
        this._paginationButton$.click(function () {
            that._loadMoreLinkedInPosts();
        });
    };

    LinkedInWidget.prototype._setUpScreenNameSelection = function () {
        var that = this;
        ...
        screenNameSelector$.change(function () {
            ...
            that._loadMoreLinkedInPosts(true);
        });
    };

    LinkedInWidget.prototype._init = function () {
        SortableWidget.prototype._init.call(this);
        if (!this.hasError) {
            this._paginationButton$ = this.element.find('.load-more-linkedInPosts-button');
