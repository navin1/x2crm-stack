...
class MassPublishNote extends MassPublish {

    public $type = 'note';

    public function getPackages () {
        return array_merge (parent::getPackages (), array (
            'MassPublishNote' => array(
                'baseUrl' => Yii::app()->request->baseUrl,
                'js' => array(
                    'js/X2GridView/MassPublishNote.js',
                ),
    ...
    public function getLabel () {
        if (!isset ($this->_label)) {
            $this->_label = Yii::t('app', 'Add note');
        }
        return $this->_label;
    }
    ...
    public function renderDialog ($gridId, $modelName) {
        echo "
            <div class='mass-action-dialog' id='".$this->getDialogId ($gridId)."'
             style='display: none;'>
                <span>".
                    Yii::t('app', 'Attach the following note to the selected records:')."
    ...
    protected function getMessages () {
        return array (
            'failed' => function ($model) { 
                return array (
                    'message' => Yii::t(
                        'app', 'Failed to add note to {record}', 
            ...
            'saved' => function ($saved) {
                return Yii::t(
                    'app', 'Note added to {n} record|Note added to {n} records',
                    array ($saved));
            },
            'unauthorized' => function ($unauthorized) {
                return Yii::t(
                    'app', 'You were not authorized to add notes to {n} record|You were not authorized to add notes to {n} records'
                        , array($unauthorized));
            },
        );
    }
