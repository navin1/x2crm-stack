...
class MassPublishAction extends MassPublish {

    public $type = '';

    public function getPackages () {
        return array_merge (parent::getPackages (), array (
            'MassPublishAction' => array(
                'baseUrl' => Yii::app()->request->baseUrl,
                'js' => array(
                    'js/X2GridView/MassPublishAction.js',
                ),
    ...
    public function getLabel () {
        if (!isset ($this->_label)) {
            $this->_label = Yii::t('app', 'Add action');
        }
        return $this->_label;
    }
    ...
    public function renderDialog ($gridId, $modelName) {
        echo "
            <div class='mass-action-dialog' id='".$this->getDialogId ($gridId)."'
             style='display: none;'>
                <span>".
                    Yii::t('app', 'Add action to the selected records:')."
    ...
    protected function getMessages () {
        return array (
            'failed' => function ($model) { 
                return array (
                    'message' => Yii::t(
                        'app', 'Failed to add action to {record}', 
            ...
            'saved' => function ($saved) {
                return Yii::t(
                    'app', 'Action added to {n} record|Action added to {n} records',array($saved));
            },
            'unauthorized' => function ($unauthorized) {
                return Yii::t(
                    'app', 'You were not authorized to add actions to {n} record|You were not authorized to add actions to {n} records'
                    , array($unauthorized));
            },
        );
    }
