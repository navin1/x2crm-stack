...
class ModuleModelNameValidator extends X2Validator {
    /**
     * @var bool $includeActions
     */
    public $includeActions = false; 

    protected function validateValue (CModel $object, $value, $attribute) {
        if (!is_string ($value) || 
            (!in_array ($value, array_keys (X2Model::getModelNames ())) &&
             (!$this->includeActions || $value !== 'Actions'))) {
