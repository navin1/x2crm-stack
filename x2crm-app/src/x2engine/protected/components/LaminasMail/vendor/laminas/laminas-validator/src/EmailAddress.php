...
class EmailAddress extends AbstractValidator
{
    ...
    public function isValid($value)
    {
        ...
        //changed since outlook can return long address do to send by or on behaf of
        if ($this->getOption('strict') && ((strlen($this->localPart) > 64) || (strlen($this->hostname) > 255)) && strpos($value, 'msonline-outlook') === false) {
            $length = false;
