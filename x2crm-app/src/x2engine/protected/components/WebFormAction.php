...
class WebFormAction extends CAction {
    ...
    public function run() {
        ...
        // Read with a plain query rather than the $webForm AR model: the
        // active/deactivateAt columns were added directly via ALTER TABLE
        // (see WhatsappGroupsController::ensureWebFormManagementColumns),
        ...
        if (isset($webForm)) {
            ...
            $scheduledPast = $deactivateAt && $deactivateAt <= time();
            if (!$active || $scheduledPast) {
                echo '<!doctype html><html><body style="font-family:sans-serif;text-align:center;' .
                    'padding:40px 20px;color:#555;"><p>' .
                    Yii::t('app', 'This form is no longer accepting submissions.') .
                    '</p></body></html>';
                Yii::app()->end();
                return;
            }
