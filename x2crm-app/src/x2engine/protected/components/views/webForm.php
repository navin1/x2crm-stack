...
body {
    /* Per requirement: always a clean white background, regardless of any
       background color an admin may have set in the form designer. */
...
label {
    /* inline-block, not block: a trailing "required" asterisk is rendered
       as a sibling *after* this tag for some fields (see renderFields()
       below) rather than nested inside it — a block label would force
       that sibling span onto its own line. The explicit <br/> already
       below every label (for 'top'-position fields) handles moving the
       input itself to the next line, so this doesn't need to be block. */
...
/* Two different code paths render a required-field asterisk here: Yii's
   own labelEx() nests <span class="required">*</span> inside the <label>
   for fields the Contacts model itself marks required (firstName,
   lastName), while renderFields() below appends a sibling
   <span class="asterisk"> *</span> after the label for fields the form
   builder marks required but the model doesn't (e.g. email). Both are
   styled identically here so it doesn't matter which path a given field
   takes — scoped to span.required specifically (not the bare .required
   class, which is also set on the <label> itself and would otherwise turn
   the whole label text red too).
*/
